...
void test_push_back() {
	{ // no resize
		vec_t *vec = vec_new(sizeof(int));
		int value = 5;
		ASSERT(!vec_push_back(vec, &value, sizeof(int)));
...
void test_push_front() {
	{ // no resize
		vec_t *vec = vec_new(sizeof(int));
		for (int i = 0; i < 7; i++)
			vec_push_back(vec, &i, sizeof(int));
		int value = -1;
...
void test_len_and_capacity() {
	...
	for (int i = 0; i < 9; i++) {
		vec_push_back(vec, &i, sizeof(int));
		ASSERT(vec_len(vec) == vec->len);
		ASSERT(vec_capacity(vec) == vec->capacity);
	}
	vec_del(vec);
}
...
void test_insert() {
	{ // no resize
		...
		for (int i = 0; i < 7; i++)
			vec_push_back(vec, &i, sizeof(int));
		int value = 5;
		vec_insert(vec, &value, 2, sizeof(int));
	...
	{ // resize
		...
		for (int i = 0; i < 8; i++)
			vec_push_back(vec, &i, sizeof(int));
		int value = 5;
		vec_insert(vec, &value, 4, sizeof(int));
...
void test_vec_at() {
	{ // no resize
		...
		for (int i = 0; i < 10; i++) {
			vec_push_back(vec, &i, sizeof(int));
			int value = 0;
			vec_at(vec, &value, (size_t)i, sizeof(int));
			ASSERT(value == i);
		}
		vec_del(vec);
	}
...
void test_append() {
	{ // no resize
		...
		ASSERT(vec->capacity == DEFAULT_CAPACITY);
		int arr[] = {5, 6, 7};
		vec_append(vec, arr, 3, sizeof(int));
		ASSERT(vec->capacity == DEFAULT_CAPACITY);
		for (int i = 0; i < 8; i++) {
			int value = -1;
			vec_at(vec, &value, (size_t)i, sizeof(int));
	...
	{ // arr len 0
		...
		ASSERT(vec->capacity == DEFAULT_CAPACITY);
		int arr[0];
		vec_append(vec, arr, 0, sizeof(int));
		ASSERT(vec->capacity == DEFAULT_CAPACITY);
		for (int i = 0; i < 5; i++) {
			int value = -1;
			vec_at(vec, &value, (size_t)i, sizeof(int));
...
void test_prepend() {
	{ // no resize
		...
		ASSERT(vec->capacity == DEFAULT_CAPACITY);
		int arr[] = {0, 1, 2};
		vec_prepend(vec, arr, 3, sizeof(int));
		ASSERT(vec->capacity == DEFAULT_CAPACITY);
		for (int i = 0; i < 8; i++) {
			int value = -1;
			vec_at(vec, &value, (size_t)i, sizeof(int));
	...
	{ // resize
		...
		vec_prepend(vec, arr, 8, sizeof(int));
		ASSERT(vec->capacity == DEFAULT_CAPACITY * 2);
		for (int i = 0; i < 13; i++) {
			int value = -1;
			vec_at(vec, &value, (size_t)i, sizeof(int));
	...
	{ // no resize
		...
		ASSERT(vec->capacity == DEFAULT_CAPACITY);
		int arr[0];
		vec_prepend(vec, arr, 0, sizeof(int));
		ASSERT(vec->capacity == DEFAULT_CAPACITY);
		for (int i = 0; i < 5; i++) {
			int value = -1;
			vec_at(vec, &value, (size_t)i, sizeof(int));
...
void test_replace() {
	...
	int value = 3;
	vec_replace(vec, &value, 3, sizeof(int));
	for (int i = 0; i < 8; i++) {
		value = -1;
		vec_at(vec, &value, (size_t)i, sizeof(int));
		ASSERT(value == i);
...
void test_replace_range() {
	{ // shrink
		...
		vec_append(vec, arr1, 8, sizeof(int));
		ASSERT(vec->len == 8);
		int arr2[] = {3, 4};
		vec_replace_range(vec, arr2, 2, 3, 3, sizeof(int));
		ASSERT(vec->len == 7);
		for (int i = 0; i < 7; i++) {
			int value = -1;
			vec_at(vec, &value, (size_t)i, sizeof(int));
	...
	{ // expand
		...
		vec_append(vec, arr1, 6, sizeof(int));
		ASSERT(vec->len == 6);
		int arr2[] = {3, 4, 5, 6, 7, 8};
		vec_replace_range(vec, arr2, 6, 3, 1, sizeof(int));
		ASSERT(vec->len == 11);
		for (int i = 0; i < 11; i++) {
			int value = -1;
			vec_at(vec, &value, (size_t)i, sizeof(int));
	...
	{ // len 0
		vec_t *vec = vec_new(sizeof(int));
		int arr1[] = {0, 1, 2, 3, 4};
		vec_append(vec, arr1, 5, sizeof(int));
		ASSERT(vec->len == 5);
		int arr2[0];
		vec_replace_range(vec, arr2, 0, 3, 0, sizeof(int));
		ASSERT(vec->len == 5);
		for (int i = 0; i < 5; i++) {
			int value = -1;
			vec_at(vec, &value, (size_t)i, sizeof(int));
	...
	{ // range 0
		...
		ASSERT(vec->len == 3);
		int arr2[] = {2, 3};
		vec_replace_range(vec, arr2, 2, 2, 0, sizeof(int));
		ASSERT(vec->len == 5);
		for (int i = 0; i < 5; i++) {
			int value = -1;
			vec_at(vec, &value, (size_t)i, sizeof(int));
